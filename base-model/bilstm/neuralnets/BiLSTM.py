...
class BiLSTM:
    ...
    def computeScore(self, modelName, devMatrix, testMatrix):
        ...
        if self.labelKeys[modelName].endswith('_BIO') or self.labelKeys[modelName].endswith('_IOBES') or self.labelKeys[modelName].endswith('_IOB'):
            ...
            return self.computeF1MacroScores(modelName, devMatrix, testMatrix)   
    ...
    def computeF1MacroScores(self, modelName, devMatrix, testMatrix):
        dev_acc = self.compute_f1_macro(modelName, devMatrix)
        test_acc = self.compute_f1_macro(modelName, testMatrix)
    ...
    def computeAcc(self, modelName, sentences):
        ...
        return float(numCorrLabels)/numLabels
    
    def vectorize_result(self, labels, data):
        result = []
        for d in data:
          for i in range(len(d)):
            line_result = [0] * len(labels)
            idx = labels.index(d[i])
            line_result[idx] = 1
            result.append(line_result[1:])
        return(result)
    
    def compute_f1_macro(self, modelName, sentences):
        correctLabels = [sentences[idx][self.labelKeys[modelName]] for idx in range(len(sentences))]
        predLabels = [p.tolist() for p in self.predictLabels(self.models[modelName], sentences)]

        labels = [1,2,3,4,5,6,7,8]

        # l_correctLabels = self.vectorize_result([0] + labels, correctLabels)
        # l_predLabels = self.vectorize_result([0] + labels, predLabels)

        mlb = MultiLabelBinarizer()
        
        mlb.fit([labels])
        
        mlb_correctLabels = mlb.transform(correctLabels)
        mlb_predLabels = mlb.transform(predLabels)
        
        """     
        mlb_correctLabels = mlb.transform(l_correctLabels)
        mlb_predLabels = mlb.transform(l_predLabels)
        """
        
        score = f1_score(mlb_correctLabels, mlb_predLabels, average = 'macro')
